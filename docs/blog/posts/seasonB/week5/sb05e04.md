...
Pour rappel nous avons déjà défini notre *Model* `Coffee` en cours hier ainsi que le *Model* `Country` lors du challenge (voir [📅 SAISON B Semaine 5 Épisode 3 - Introduction aux ORM](./sb05e03.md)).
...
1. **`sequelize.sync({ force: true, logging: false })`**
   + **`force: true`** : Supprime et recrée **toutes les tables** de la base de données.
   + **`logging: false`** : Désactive les logs SQL pour éviter la pollution de la console.

2. **`Country.bulkCreate(data.countries, { returning: true })`**
   + **`bulkCreate`** : Crée plusieurs enregistrements en une seule requête (optimisé pour les performances).
   + **`returning: true`** : Retourne les instances créées (utile pour récupérer les IDs générés).

3. **Création des Cafés avec leurs Pays Associés**
...
4. **Association des Catégories aux Cafés**
...
Nous allons pratiquer dans le [challenge du jour](#️-challenge-du-jour--projet-ocoffee--projet-fil-rouge-suite) 😊.
